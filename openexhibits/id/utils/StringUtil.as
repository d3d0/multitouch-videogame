...
public class StringUtil
{
	...
	*/
	...
	public static function between(value:String, patternStart:String, patternEnd:String, startIndex:int = 0, endIndex:int = int.MAX_VALUE):String
	{
		if (startIndex && (startIndex >= value.length || endIndex <= startIndex))
		{
			throw new ArgumentError("StringUtil::between - endIndex cannot be less then the startIndex!");
		}
		...
		var pStartIndex:int = !isEmpty(patternStart) ? value.indexOf(patternStart, startIndex) : 0 ;
		if (pStartIndex == -1)
		{
			return null;
		}
		
		pStartIndex += patternStart.length;
		
		var pEndIndex:int = !isEmpty(patternEnd) ? value.indexOf(patternEnd, pStartIndex) : value.length ;
		if (pEndIndex == -1 || pEndIndex > endIndex)
		{
			return null;
		}

		return value.substr(pStartIndex, pEndIndex - pStartIndex);
	}
    ...
    */
    ...
    public static function padCenter(p_string:String, p_padChar:String, p_length:uint):String
    {
        var s:String = p_string;
        var l:Boolean = true;
    ...
    */
	...
	public static function tokenize(value:String, format:String):Object
	{
		var idx:uint;
		...
		for(idx=0; idx<matches.length; idx++)
		{
			...
			deliminators.push
			(
				StringUtil.between(format, matches[idx], matches[idx + 1], position)
			);
			
			position = format.indexOf(matches[idx]) + matches[idx].length;
		}
		...
		for(idx=0; idx<matches.length; idx++)
		{
			...
			patternStart = idx ? deliminators[idx - 1] : "" ;
			patternEnd = idx != matches.length ? deliminators[idx] : "" ;
			
			if(value.indexOf(patternEnd, position + patternStart.length) == -1)
			{
				patternEnd = "";
			}
			
			result = StringUtil.between(value, patternStart, patternEnd, position);
			results[matches[idx].replace(/[<>]/g, "")] = result;
			
			position += patternStart.length + result.length;
		}
